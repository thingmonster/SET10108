...
void trap(double start, double end, size_t iterations, shared_ptr<double> p)
{
    // trying to pass in a lambda function produces the following error
    // error: variable or field 'trap' declared void
    // so it's now declared with this function
...
int main(int argc, char **argv)
{
    ...
    auto trapezoids = static_cast<size_t>(pow(2, 11));
    // Get number of threads
...
#pragma omp parallel num_threads(thread_count)
    trap(start, end, trapezoids, result);
